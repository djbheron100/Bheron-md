...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t Logo by BHERON_Md v5*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by BHERON_md *" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
repondre("  processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by BHERON MD*"})
...
zokou({ nomCom: "snow", categorie: "Logo", reaction: "❄️" }, async (dest, zk, commandeOptions) => {
  ...
  mumaker.textpro("https://textpro.me/create-beautiful-3d-snow-text-effect-online-1101.html", text)
    .then((data) => {
      zk.sendMessage(dest, { image: { url: data.image }, caption: 'Logo by BHERON MD' }, { quoted: ms });
    })
    .catch((err) => {
...
zokou({nomCom:"purple",categorie:"Logo",reaction:"🧳"},async(dest,zk,commandeOptions)=>{
      ...
      if(!arg||arg=="")
  {
    repondre(prefixe+"purple zokou");return;}
      ...
      var img = await mumaker.ephoto(lien,arg);
      repondre(" processing ...")
      await zk.sendMessage(dest,{image:{url:img.image},caption:"      *Logo by BHERON_MD*"});
    }catch(e){repondre(e)}
...
zokou({ nomCom: "gif×1", categorie: "Logo", reaction: "😋" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    repondre("Processing...");
    await zk.sendMessage(dest, { image: { url: img }, caption: "*Logo by BHERON-Md*" }, { quoted: ms });
  } catch (e) {
    repondre(e);
  }
});
